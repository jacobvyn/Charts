...
class LineChartData extends AxisChartData {
  ...
  final TitlesData titlesData;
  final ExtraLinesData extraLinesData;
  final LineTouchData lineTouchData;

  LineChartData({
    this.lineBarsData = const [],
    this.titlesData = const TitlesData(),
...
typedef CheckToShowSpotBelowLine = bool Function(Spot spot);

bool showAllSpotsBelowLine(Spot spot) {
  return true;
}
...
class BelowSpotsLine {
  ...
  const BelowSpotsLine({
    this.show = false,
    this.lineStyle = const Line(),
    this.checkToShowSpotBelowLine = showAllSpotsBelowLine,
  });
}

/***** DotData *****/
typedef CheckToShowDot = bool Function(Spot spot);

bool showAllDots(Spot spot) {
  return true;
}
...
List<TouchedSpotIndicatorData> defaultTouchedIndicators(
  ...
  return touchedSpots.map((LineTouchedSpot lineTouchedSpot) {
    ...
    final Line line = Line(color: lineColor, strokeWidth: lineStrokeWidth);
    ...
    DotData dotData = DotData(
      dotSize: dotSize,
      dotColor: dotColor,
    );

    return TouchedSpotIndicatorData(line, dotData);
  }).toList();
}

/// holds data for handling touch events on the [LineChart]
class LineTouchData extends TouchData {
...
class TouchedSpotIndicatorData {
  final Line indicatorBelowLine;
  final DotData touchedSpotDotData;
