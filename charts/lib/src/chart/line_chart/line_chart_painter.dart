...
import 'package:charts/src/chart/base/axis_chart/axis_chart_data.dart';
import 'package:charts/src/chart/base/axis_chart/axis_chart_painter.dart';
...
class LineChartPainter extends AxisChartPainter {
  ...
  LineTouchedSpot _getNearestTouchedSpot(
    ...
    for (Spot spot in barData.spots) {
  ...
  void drawDots(Canvas canvas, Size viewSize, LineChartBarData barData) {
    ...
    barData.spots.forEach((spot) {
      if (barData.dotData.checkToShowDot(spot)) {
        double x = getPixelX(spot.x * animation.value, viewSize);
        double y = getPixelY(spot.y * animation.value, viewSize);
  ...
  Path _generateBarPath(Size viewSize, LineChartBarData barData) {
    ...
    for (int i = 1; i < size; i++) {
      ...
      final current = Offset(
        getPixelX(barData.spots[i].x * animation.value, viewSize),
        getPixelY(barData.spots[i].y * animation.value, viewSize),
      );

      /// previous spot
      final previous = Offset(
        getPixelX(barData.spots[i - 1].x * animation.value, viewSize),
        getPixelY(barData.spots[i - 1].y * animation.value, viewSize),
      );

      /// next point
      final next = Offset(
        getPixelX(barData.spots[i + 1 < size ? i + 1 : i].x * animation.value,
            viewSize),
        getPixelY(barData.spots[i + 1 < size ? i + 1 : i].y * animation.value,
            viewSize),
      );
      ...
      for (Spot spot in barData.spots) {
        if (barData.belowBarData.belowSpotsLine.show &&
          ...
          belowBarLinePaint.color =
              barData.belowBarData.belowSpotsLine.lineStyle.color;
          belowBarLinePaint.strokeWidth =
              barData.belowBarData.belowSpotsLine.lineStyle.strokeWidth;
  ...
  void drawBar(
    ...
    if (barData.colors.length == 1) {
      barPaint.color = barData.colors.first;
